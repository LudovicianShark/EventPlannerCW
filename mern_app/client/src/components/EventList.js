                    ...
                    {items.map(({ _id, name, type, eventDate, startTime, endTime, description}) => (
                        <CSSTransition key={_id} timeout={500} classNames="fade">
                            <ListGroupItem>
                                ...
                                <h6>Event Date: {Moment({eventDate}).format('DD/MM/YYYY')}</h6> <p className="desc-p">Start: {startTime} </p> <p> Finish: {endTime}</p> <p className="desc-p">Description: </p><p>{description}</p>
                            </ListGroupItem>
                        </CSSTransition>
                    ))}
                </TransitionGroup>
